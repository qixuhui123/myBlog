...
function queryByTag(request, response) {
    ...
    tagsDao.queryTag(params.tag,function (result) {
        if(result == null || result.length == 0 ){
            ...
            tagsBlogMappingDao.queryByTag(result[0].id,parseInt(params.page),parseInt(params.pageSize) ,function (result) {
                var blogList = [];
...
function queryByTagCount(request, response) {
    var params = url.parse(request.url,true).query;
    tagsDao.queryTag(params.tag,function (result) {
        tagsBlogMappingDao.queryByTagCount(result[0].id, function () {
            response.writeHead(200);
            response.write(respUtil.writeResult("success" , "评论成功",result));
            response.end();
        })
